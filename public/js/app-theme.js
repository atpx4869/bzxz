// app-theme.js — 主题切换(dark / light / paper),所有用户可用
//
// 设计:
//  - 三主题:dark(默认) / light(Arctic Blue 蓝调) / paper(Claude Linen 温暖印刷品)
//  - 持久化:localStorage 'bzxz.theme'
//  - 切换载体:<html data-theme="dark|light|paper">
//  - 入口:
//    - 手机「我」页:3 chip 行,3 选 1 显式切换
//    - 桌面 topbar:icon 按钮点击展开 picker(3 项菜单),不再 toggle 两态
//  - 避免 FOUC:public/index.html + web/index.html <head> 顶部加内联 script
//
// 公开 API:
//   window.bzxzTheme.get()             返回 'dark' | 'light' | 'paper'
//   window.bzxzTheme.set(theme)        切换 + persist
//   window.bzxzTheme.toggle()          legacy 接口(只在 dark<->light 切,paper 走 set)
...
(function() {
  ...
  /** legacy 接口:dark<->light 两态切换。paper 用户走 set('paper') 或 picker */
  function toggleTheme() {
    var cur = getTheme();
    if (cur === 'paper') setTheme('dark');         // paper → dark(legacy 行为兜底)
