...
|---|---|:-:|---|---|---|
| 50 | parseLibraryFilename 兼容新旧文件名 + 单测 | [ ] | | | `src/services/library-index.ts` + 新增 `library-index.test.ts`。验收：BW/BZ/BY/LB 四标签 × 含/不含 title 八种组合全过 |
...
| 57 | labr-panel.ts 业务逻辑 | [ ] | | | `web/src/labr/labr-panel.ts` + `labr-login.ts` + `labr-card.ts`。接通 /api/labr/*，tab 切到 labr 时 lazy-init |
| 58 | multi-source preview picker | [ ] | | | `web/src/preview/multi-source-picker.ts`。所有 tab 预览入口共用 |
| 59 | pages/labr.css + file-type-badge.css | [ ] | | | 暗色玻璃主题；`web/src/styles/index.css` 排好导入顺序；`SECTIONS.md` 同步 |
| 60 | 文档同步 + CI 绿 | [ ] | | | `README.md` / `web/README.md` / `docs/ARCHITECTURE.md` / `CHANGELOG.md`；本文顶部 V2 状态 → ✅ 已上线 |
