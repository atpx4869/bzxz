...
`by` / `labr` 这类需要登录的源必须通过 `.env.local` 注入凭据；仓库不再提供任何默认账号。
...
| POST | `/api/standards/multi-download` | 多源自动切源下载（按 `sourceIds` 映射和优先级依次尝试） |
...
| GET | `/api/downloads?q=&limit=&offset=` | 下载/本地文件库列表；支持服务端筛选与分页，默认取 200 条 |
...
`bzxz://` 协议只在**装了桌面版的机器**上生效。其余用户（通常只有 admin 装桌面版，其他人走网页版）机器上没有该协议处理器，应改用 **http 深链**直达同一台 admin 服务器的 web 版——前端 `initRouter` 读 `?tab=&q=` 是桌面 / 浏览器**共用同一套代码**，零额外实现：

| 协议 URL（桌面版机器） | http 深链（网页版机器） |
|---|---|
| `bzxz://search?q={query}` | `http://<admin内网IP>:5937/?tab=search&q={query}` |
| `bzxz://qual?q={query}` | `http://<admin内网IP>:5937/?tab=qual&q={query}` |

http 深链会用默认浏览器打开 web 版，**登录态不丢参**：LAN 用户先过登录浮层，登录成功（`onAuthReady → initPanels → initRouter`）后才读地址栏的 `?tab=&q=` 并触发搜索，不会提前打空请求。

**「有桌面版优先桌面版，没有就走 web」按机器分发**（不做运行时探测——是否有桌面版取决于机器装没装，与登录账号无关，且固定绑定）：

| 谁 | Listary 该配 |
|---|---|
| **admin（装了桌面版）** | `bzxz://...` 两条（唤起本机桌面 app） |
| **其他用户（仅网页版）** | `http://<admin内网IP>:5937/?tab=...&q={query}` 两条（默认浏览器打开 web 版） |

> `<admin内网IP>:5937` 就是他们平时访问网页版的同一地址（见下「手机访问」，桌面端「设置 → 网页版启动器」卡片可复制）。普通用户机器上本就没 `bzxz://`，**不要**给他们配协议链接——某些浏览器会弹"无法打开"框。
...
- PR / 手动检查走 `.github/workflows/pr-check.yml`：`npm run build` → `npm test` → `npm run oklch:check`
- 打包前自动把版本号设置为 `1.0.<github.run_number>`
- 打包 workflow 同样会跑 `npm run build`、`npm test`、`npm run oklch:check`，再执行 Electron 打包
...
- **feat(theme): 第四主题 `legacy` — Win7 / Chrome ≤109 完整兜底** — 在 dark/light/paper 之外新增第四主题，给 Windows 7（系统中 Chrome 官方支持上限 109）/ 老版本 Chromium 用户独立的视觉路径，**完全不动**现代浏览器用户看到的 frosted glass 设计资产。**自动触发**：`public/index.html` + `web/index.html` 顶部 FOUC 内联 script 检测 UA（`Chrom(e\|ium)/\d+ ≤ 109` 或 `Windows NT 5.x/6.x` 即 XP/Vista/7/8/8.1），命中即写 `localStorage 'bzxz.theme'='legacy'`。**手动触发**：topbar picker + 我页 chip 第 4 项 `◆ 经典`。**禁用清单**：legacy 主题完全不用 `oklch / color-mix / backdrop-filter / mask-image` 大区域 / Google Fonts / SMP 区彩色 emoji；纯 hex 调色板 + Microsoft YaHei 系统字体 + BMP 区几何符号（◎▤◐▣▦▥▩◈◇⚙⚐ 等用 `::before`/`::after` 注入覆盖 HTML 里的 🔍📊📥📋📑 emoji，避免 Win7 显示成方框）+ 不透明 surface 替换所有 frosted 表面。**JS 兼容性扫描**：仓库未用 `toSorted/toReversed/Promise.withResolvers/Object.groupBy/structuredClone/findLast`，无需 polyfill。**入口侵入 7 处**：`web/src/styles/theme/legacy.css`（新建）+ `index.css` `@import` 顺序在 glass.css 之后 + `public/styles.css` 末尾追加同段（迁移期双轨）+ 两个 `index.html` FOUC script + picker + 「我」页 chip + Google Fonts 条件加载 + `app-theme.js` `VALID/THEME_META` + `scripts/css-oklch-fallback.mjs` `SKIP_FILES` 白名单。维护契约见 [`CLAUDE.md`「Legacy 主题契约」](./CLAUDE.md)。
