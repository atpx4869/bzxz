...
export function createAdminRoutes(db: Database.Database) {
  ...
  // POST /api/admin/spc/cookie
  //
  // 用户在浏览器手动登录 spc.org.cn 后，从 devtools 拷 `Cookie:` header 字符串粘进来。
  // 后端把 cookie 写 settings 表（key=`spc.cookies` / `spc.cookies_expires_at`），下次
  // adapter 调 stdonline / onlinereading 复用。
  //
  // 不校验 cookie 是否真实可用 —— 校验需要再发一次 stdonline 请求，user 等不及；
  // 下次导出失败时统一抛 BadRequestError「spc 凭据失效，请重新粘贴 Cookie」。
  //
  // 暴露在 admin（非 settings 普通页面）避免普通用户误改全局 session。
  router.post('/spc/cookie', async (req, res, next) => {
    try {
      const schema = z.object({
        cookie: z.string().min(10).max(8192),
        // 寿命默认 6h，admin 可显式覆盖（已知 cookie 寿命未知，未来实测后调）
        lifetimeHours: z.number().min(1).max(720).optional(),
      });
      const { cookie, lifetimeHours } = schema.parse(req.body || {});
      const { getSpcAdapter } = require('../sources/spc/spc-adapter');
      const adapter = getSpcAdapter();
      const lifetimeMs = (lifetimeHours ?? 6) * 3600_000;
      const session = adapter.setSessionFromCookie(cookie, lifetimeMs);
      respond(res, {
        ok: true,
        expiresAt: new Date(session.expiresAt).toISOString(),
        lifetimeHours: lifetimeHours ?? 6,
      });
    } catch (error) {
      next(normalizeError(error));
    }
  ...
  // DELETE /api/admin/spc/cookie
  //
  // 主动清空 spc cookie（用户报告失效 / 想强制重粘）。等同 spc-adapter 的 invalidateSession。
  router.delete('/spc/cookie', async (_req, res, next) => {
    try {
      setSetting(db, 'spc.cookies', '');
      setSetting(db, 'spc.cookies_expires_at', '0');
      // adapter 进程内缓存也清掉，避免下次导出仍用旧 cookie 拉
      const { getSpcAdapter } = require('../sources/spc/spc-adapter');
      const adapter = getSpcAdapter();
      if (typeof adapter._clearMemorySession === 'function') {
        adapter._clearMemorySession();
      }
      respond(res, { ok: true });
    } catch (error) {
      next(normalizeError(error));
  ...
  // GET /api/admin/spc/cookie
  //
  // 诊断用：返回 cookie 是否已配置 + 过期时间（不返回 cookie 真值，防止泄漏）。
  router.get('/spc/cookie', (_req, res, next) => {
    try {
      const cookies = getSetting(db, 'spc.cookies', '');
      const expRaw = getSetting(db, 'spc.cookies_expires_at', '0');
