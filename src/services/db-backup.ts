 ...
 * DB 自动备份 + 缺失自愈
 *
 * 背景：commit 0bd54c4 之前的 installer.nsh 没保留 $INSTDIR\data，从更早版本
 * 升级的用户会被旧卸载器 RMDir /r 把 users / 资质数据全部抹掉。装机版升级
 * 涉及 NSIS 卸载器的版本永远是「上一次安装时落盘的卸载器」 —— 即使我们现在
 * 修了 installer.nsh，老 exe 自带的卸载器逻辑无法追溯。
 *
 * 因此这里加一层 application-level 防御：
 *   - 备份位置：%APPDATA%\bzxz\bzxz-db-backups\（NSIS 永远不动 userData）
 *   - 启动时若 db 不存在或 0 字节 → 从最新 backup 还原
 *   - 启动正常完成后异步生成新备份，保留最近 N 份，更老的删掉
 *
 * 用 better-sqlite3 的 db.backup() API（online backup，对 WAL 安全），不直接
 * cp 数据文件 —— 数据文件可能正在 checkpoint，硬拷会拿到不一致快照。
 */
...
/**
 * 备份文件存放目录。优先 BZXZ_USER_DATA_DIR（Electron 主进程设的 userData）；
 * dev 模式 fallback 到 cwd/data/backups（dev 不存在升级丢数据风险，仅为路径自洽）。
 */
function getBackupDir(): string {
  const userData = process.env.BZXZ_USER_DATA_DIR;
  if (userData) return path.join(userData, BACKUP_DIR_NAME);
