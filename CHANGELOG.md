...
- **#64 spc.org.cn 第 5 数据源接入（纯 HTTP 链路 + 手动 Cookie 路径）**：用户要求接入 spc.org.cn 作为第 5 个源，与 bz/gbw/by 并列挂主搜索 + 库绿点 + 批量下载流。前期勘察（`out/spc-chain-summary.md`、`out/spc-sample-09a6fd0484f5.pdf`）证实：服务端一次性下发完整无水印 PDF（430KB / 20 页样本），4 步纯 HTTP 请求即可下载，比浏览器自动化快 10×。本次：
  - **新增 `src/sources/spc/spc-client.ts`**：协议层 stateless HTTP 客户端，方法 `searchByKeyword / getReaderToken / downloadPdf / submitLogin / getCaptcha`。纯函数 `stripHighlightTags`（剥 queryfocus `<font>` 高亮）、`inferStandclass`（从 a100 前缀映射 CN/ISO/GW）、`extractTokenFromHtml`（抠 stdonline 返回 HTML 里的 `var rc = "..."`）、`mergeCookies / parseSetCookieList`。**字节通道关键：`downloadPdf` 必须 `arrayBuffer()`**，Playwright/Chromium 因 `Content-Type: application/pdf;charset=utf-8` 会按 UTF-8 解码破坏 PDF，但 Node undici 不看 charset 永远返回原始字节（这是纯 HTTP 路径成立的核心）。
  - **新增 `src/sources/spc/spc-adapter.ts`**：实现 `SourceAdapter`，仿 by-adapter 套路。Token 单次有效 → `detectPreview` 不预拉，仅返回 `previewAvailable: true, pageUrls: []`；`exportStandard` 内部串联 stdonline → onlinereading → addFileToLibrary。`sourceId = ${a100}|${standclass}`（ISO 含 `:` 替换为 `∶` 避开 createStandardId 禁忌），落库时按 `cleanStdCode` 三层归一化契约处理。Cookie 失效抛 `SpcAuthError` → adapter 清 settings + 抛 `BadRequestError`（"spc 凭据失效，请在 admin 面板重新粘贴 Cookie"）。
  - **凭据策略**：submitlogin 需要 4 字母图形验证码（OCR 难度高），MVP 不做 OCR 自动登录。默认走「用户在浏览器手动登录后，从 devtools 拷 Cookie 粘到 admin 面板」路径，cookie 写 settings 表（key=`spc.cookies` / `spc.cookies_expires_at`，6 小时寿命）。`SpcAdapter.setSessionFromCookie` 是 adapter 唯一对外写 session 的入口。
  - **点状改动**：`SourceName` 加 `'spc'`、`VALID_SOURCES` 加 `'spc'`、`SourceRegistry FACTORIES` 加 `spc`、`SUPPORTED_SOURCES` 加 `'spc'`、`SOURCE_LABEL_TO_CANONICAL` 加 `SPC↔spc`、`CANONICAL_TO_LABEL` 加 `spc:'SPC'`、`SOURCE_LABELS` 加 `spc:'SPC'`、`sourceEnum`/`ALL_LIBRARY_SOURCES`/`allSources`/`library_source_priority` 过滤器 4 处加 `'spc'`、`getSourceSemaphore` defaults 加 `spc:2`。前端 `public/js/app-search.js` 的 `sourceLabel` dict 加 `spc:'SPC'`。
  - **单测覆盖**：`src/sources/spc/spc-client.test.ts` 覆盖纯函数解析层（stripHighlightTags / inferStandclass / extractTokenFromHtml / mergeCookies）≈ 30 个 case。网络方法走 adapter 集成测试。
  - **凭据约定**：仍按 CLAUDE.md 强制契约 —— `SPC_USERNAME / SPC_PASSWORD` 仅放 `.env.local`（gitignored），勘察脚本和未来 OCR-自动登录路径用；MVP cookie 路径不依赖。
